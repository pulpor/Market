/**
 * Serviço para buscar dados reais do Banco Central do Brasil
...
async function fetchBCBData(
  ...
  try {
    ...
    console.log(`Buscando série ${serieId} do BCB...`);
    ...
    console.log(`✅ Série ${serieId} carregada: ${Array.isArray(data) ? data.length : 0} registros`);
...
export async function getSELICData(months: number = 12): Promise<IndexHistory[]> {
  ...
  console.log(`📡 Buscando SELIC (série 4189) de ${dataInicio} a ${dataFim}`);

  const data = await fetchBCBData(4189, dataInicio, dataFim);

  console.log(`✅ SELIC retornou ${data.length} registros`);
...
export async function getCDIData(months: number = 12): Promise<IndexHistory[]> {
  ...
  console.log(`📡 Buscando CDI (série 12) de ${dataInicio} a ${dataFim}`);

  const data = await fetchBCBData(12, dataInicio, dataFim);

  console.log(`✅ CDI retornou ${data.length} registros`);

  if (data.length === 0) {
    console.warn("⚠️ CDI vazio, usando fallback");
    return [];
  }
