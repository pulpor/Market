...
import { Plus, Trash2, Calculator, Coins, Wallet, Briefcase, ChevronLeft, ChevronRight } from "lucide-react";
...
export function ReturnsForecastSection(props: Props) {
  ...
  return (
    <div className="bg-card p-6 rounded-xl border border-border mt-8 shadow-sm">
      <h2 className="text-2xl font-bold text-foreground mb-6 flex items-center gap-2"><Calculator className="h-6 w-6" /> Previsão de Retornos</h2>
        ...
        <TabsContent value="dividends" className="mt-6 space-y-4">
          ...
          {state.dividends.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {state.dividends.slice().reverse().slice((divPage - 1) * ITEMS_PER_PAGE, divPage * ITEMS_PER_PAGE).map(d => (
              ...
              {state.dividends.length > ITEMS_PER_PAGE && (
                <div className="flex items-center justify-center gap-4 mt-4">
                  ...
                  <span className="text-sm text-muted-foreground">
                    Página {divPage} de {Math.ceil(state.dividends.length / ITEMS_PER_PAGE)}
                  </span>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setDivPage(p => Math.min(Math.ceil(state.dividends.length / ITEMS_PER_PAGE), p + 1))}
                    disabled={divPage === Math.ceil(state.dividends.length / ITEMS_PER_PAGE)}
                  >
